...
from Model.model_1unet import KnowSAM
...
class Trainer(nn.Module):
    def __init__(self, args):
        ...
        self.optimizer_SGDL = torch.optim.SGD(self.SGDL.parameters(), lr=args.UNet1_lr, momentum=0.9,
                                              weight_decay=0.0001)
    ...
    def mix_up(self, fusion_map_soft, volume_batch, pseudo_label, labeled_label, consistency_weight, patch_size=4,
        ...
        pred_UNet1_mix, pred_UNet1_soft_mix = self.SGDL(volume_batch_mix)

        pseudo_label_mix = torch.argmax(pred_UNet1_mix, dim=1)

        UNet1_sup_mixed_loss = ce_loss(pred_UNet1_mix, label_batch_mix.long()) + self.dice_loss(pred_UNet1_soft_mix, label_batch_mix)
        UNet1_enp_mixed_loss = self.entropy_loss(pred_UNet1_soft_mix, C=2)
        UNet1_unsup_mixed_loss = ce_loss(pred_UNet1_mix[self.args.labeled_bs:], pseudo_label_mix[self.args.labeled_bs:].long()) + self.dice_loss(pred_UNet1_soft_mix[self.args.labeled_bs:], pseudo_label_mix[self.args.labeled_bs:])

        UNet1_mixed_loss = UNet1_sup_mixed_loss + 0.9 * UNet1_enp_mixed_loss + consistency_weight * UNet1_unsup_mixed_loss

        return UNet1_mixed_loss

    def train(self, volume_batch, label_batch, iter_num):
        image_embeddings = self.sam_model.image_encoder(volume_batch)
        pred_UNet1, pred_UNet1_soft = self.SGDL(volume_batch)
        ...
        for i in range(self.args.num_classes):
            sparse_embeddings, dense_embeddings = self.sam_model.prompt_encoder(
                ...
                masks=F.interpolate(pred_UNet1[:, i, ...].unsqueeze(1).clone().detach(), size=(64, 64), mode='bilinear')
            )
        ...
        UNet1_sup_loss = ce_loss(pred_UNet1[:self.args.labeled_bs], label_batch[:self.args.labeled_bs].long()) + self.dice_loss(pred_UNet1_soft[:self.args.labeled_bs], label_batch[:self.args.labeled_bs])
        UNet1_enp_loss = self.entropy_loss(pred_UNet1_soft, C=2)
        UNet1_kd_loss = self.KDLoss(pred_UNet1.permute(0, 2, 3, 1).reshape(-1, 2), pred_sam.clone().detach().permute(0, 2, 3, 1).reshape(-1, 2))
        ...
        UNet1_loss = UNet1_sup_loss + UNet1_kd_loss + 0.9 * UNet1_enp_loss

        if iter_num > self.args.mixed_iterations:
            UNet1_mixed_loss = self.mix_up(pred_UNet1_soft, volume_batch, pred_sam_soft[self.args.labeled_bs:], label_batch[:self.args.labeled_bs], consistency_weight)
            SGDL_loss = UNet1_loss + UNet1_mixed_loss
        else:
            SGDL_loss = UNet1_loss
        ...
        return {
            ...
            "UNet1_loss": UNet1_loss.item(),
            "UNet1_sup_loss": UNet1_sup_loss.item(),
            "UNet1_enp_loss": UNet1_enp_loss.item(),
            "UNet1_kd_loss": UNet1_kd_loss.item(),
    ...
    def val(self, val_loader, snapshot_path, iter_num):
        ...
        for i_batch, sampled_batch in enumerate(val_loader):
            ...
            pred_UNet1, pred_UNet1_soft = self.SGDL(val_image)
            ...
            dice_SGDL = dice_coef(val_label, pred_UNet1_soft, thr=0.5)
