...
from Model.model_2vnet import KnowSAM
...
class Trainer(nn.Module):
    ...
    def mix_up(self, fusion_map_soft, volume_batch, pseudo_label, labeled_label, consistency_weight, patch_size=4,
        ...
        pred_VNet1_mix, pred_VNet2_mix, pred_VNet1_soft_mix, pred_VNet2_soft_mix, fusion_map_mix = self.SGDL(volume_batch_mix)
        ...
        VNet1_cons_mixed_loss = loss_diff1(pred_VNet1_soft_mix, pred_VNet2_soft_mix.clone().detach())
        ...
        VNet2_sup_mixed_loss = ce_loss(pred_VNet2_mix, label_batch_mix.long()) + self.dice_loss(pred_VNet2_soft_mix, label_batch_mix)
        VNet2_enp_mixed_loss = self.entropy_loss(pred_VNet2_soft_mix, C=2)
        VNet2_cons_mixed_loss = loss_diff2(pred_VNet2_soft_mix, pred_VNet1_soft_mix.clone().detach())
        VNet2_unsup_mixed_loss = ce_loss(pred_VNet2_mix[self.args.labeled_bs:], pseudo_label_mix[self.args.labeled_bs:].long()) + self.dice_loss(pred_VNet2_soft_mix[self.args.labeled_bs:], pseudo_label_mix[self.args.labeled_bs:])

        fusion_mixed_loss = ce_loss(fusion_map_mix, label_batch_mix.long()) + self.dice_loss(fusion_map_soft_mix, label_batch_mix)

        VNet1_mixed_loss = VNet1_sup_mixed_loss + 0.9 * VNet1_enp_mixed_loss + consistency_weight * (VNet1_cons_mixed_loss + VNet1_unsup_mixed_loss)
        VNet2_mixed_loss = VNet2_sup_mixed_loss + 0.9 * VNet2_enp_mixed_loss + consistency_weight * (VNet2_cons_mixed_loss + VNet2_unsup_mixed_loss)

        return VNet1_mixed_loss, VNet2_mixed_loss, fusion_mixed_loss

    def train(self, volume_batch, label_batch, iter_num):
        image_embeddings = self.sam_model.image_encoder(volume_batch)
        pred_VNet1, pred_VNet2, pred_VNet1_soft, pred_VNet2_soft, fusion_map = self.SGDL(volume_batch)
        ...
        VNet1_cons_loss = loss_diff1(pred_VNet1_soft, pred_VNet2_soft.clone().detach())
        ...
        VNet2_sup_loss = ce_loss(pred_VNet2[:self.args.labeled_bs], label_batch[:self.args.labeled_bs].long()) + self.dice_loss(pred_VNet2_soft[:self.args.labeled_bs], label_batch[:self.args.labeled_bs])
        VNet2_cons_loss = loss_diff2(pred_VNet2_soft, pred_VNet1_soft.clone().detach())
        VNet2_enp_loss = self.entropy_loss(pred_VNet2_soft, C=2)
        VNet2_kd_loss = self.KDLoss(pred_VNet2.permute(0, 2, 3, 1).reshape(-1, 2), pred_sam.clone().detach().permute(0, 2, 3, 1).reshape(-1, 2))
        ...
        VNet1_loss = VNet1_sup_loss + VNet1_kd_loss + 0.9 * VNet1_enp_loss + consistency_weight * VNet1_cons_loss
        VNet2_loss = VNet2_sup_loss + VNet2_kd_loss + 0.9 * VNet2_enp_loss + consistency_weight * VNet2_cons_loss

        if iter_num > self.args.mixed_iterations:
            VNet1_mixed_loss, VNet2_mixed_loss, fusion_mixed_loss = self.mix_up(fusion_map_soft, volume_batch, pred_sam_soft[self.args.labeled_bs:], label_batch[:self.args.labeled_bs], consistency_weight)
            SGDL_loss = (VNet1_loss + VNet1_mixed_loss + VNet2_loss + VNet2_mixed_loss) / 2 + fusion_loss + fusion_mixed_loss
        else:
            SGDL_loss = (VNet1_loss + VNet2_loss) / 2 + fusion_loss
        ...
        return {
            ...
            "VNet1_VNet2_loss": ((VNet1_loss + VNet2_loss) / 2).item(),
            "fusion_loss": fusion_loss.item(),
            "VNet1_loss": VNet1_loss.item(),
            "VNet2_loss": VNet2_loss.item(),
            "VNet1_sup_loss": VNet1_sup_loss.item(),
            "VNet2_sup_loss": VNet2_sup_loss.item(),
            "VNet1_cons_loss": VNet1_cons_loss.item(),
            "VNet2_cons_loss": VNet2_cons_loss.item(),
            "VNet1_enp_loss": VNet1_enp_loss.item(),
            "VNet2_enp_loss": VNet2_enp_loss.item(),
            "VNet1_kd_loss": VNet1_kd_loss.item(),
            "VNet2_kd_loss": VNet2_kd_loss.item(),
    ...
    def val(self, val_loader, snapshot_path, iter_num):
        ...
        for i_batch, sampled_batch in enumerate(val_loader):
            ...
            pred_VNet1, pred_VNet2, pred_VNet1_soft, pred_VNet2_soft, fusion_map = self.SGDL(val_image)
            ...
            dice_vnet1 = dice_coef(val_label, pred_VNet1_soft, thr=0.5)
            avg_dice_vnet1 += dice_vnet1
            dice_vnet2 = dice_coef(val_label, pred_VNet2_soft, thr=0.5)
            avg_dice_vnet2 += dice_vnet2

        avg_dice_sam = avg_dice_sam / len(val_loader)
        avg_dice_SGDL = avg_dice_SGDL / len(val_loader)
        avg_dice_vnet1 = avg_dice_vnet1 / len(val_loader)
        avg_dice_vnet2 = avg_dice_vnet2 / len(val_loader)
        ...
        logging.info('iteration %d : '
                     ...
                     '  vnet1_mean_dice : %f '
                     '  vnet2_mean_dice : %f '
                    % (iter_num, avg_dice_sam, avg_dice_SGDL, avg_dice_vnet1, avg_dice_vnet2))
        ...
        return {
            ...
            "vnet1_mean_dice": float(avg_dice_vnet1),
            "vnet2_mean_dice": float(avg_dice_vnet2)
        }