...
class KnowSAM(nn.Module):
    def __init__(self, args, bilinear=False, has_dropout=False):
        ...
        self.has_dropout = has_dropout
        self.vnet_1 = VNet(n_channels=args.in_channels, n_classes=args.num_classes)
        self.vnet_2 = VNet(n_channels=args.in_channels, n_classes=args.num_classes)
    ...
    def forward(self, x):
        # vnet_1 encoder
        x0_v1 = self.vnet_1.block_one(x)
        x1_v1 = self.vnet_1.block_one_dw(x0_v1)
        x2_v1 = self.vnet_1.block_two_dw(self.vnet_1.block_two(x1_v1))
        x3_v1 = self.vnet_1.block_three_dw(self.vnet_1.block_three(x2_v1))
        x4_v1 = self.vnet_1.block_four_dw(self.vnet_1.block_four(x3_v1))

        # vnet_2 encoder
        x0_v2 = self.vnet_2.block_one(x)
        x1_v2 = self.vnet_2.block_one_dw(x0_v2)
        x2_v2 = self.vnet_2.block_two_dw(self.vnet_2.block_two(x1_v2))
        x3_v2 = self.vnet_2.block_three_dw(self.vnet_2.block_three(x2_v2))
        x4_v2 = self.vnet_2.block_four_dw(self.vnet_2.block_four(x3_v2))

        # vnet_1 decoder
        x_v1 = self.vnet_1.block_five_up(self.vnet_1.block_five(x4_v1))
        x_v1 = x_v1 + x3_v1
        x_v1 = self.vnet_1.block_six_up(self.vnet_1.block_six(x_v1))
        x_v1 = x_v1 + x2_v1
        x_v1 = self.vnet_1.block_seven_up(self.vnet_1.block_seven(x_v1))
        x_v1 = x_v1 + x1_v1
        x_v1 = self.vnet_1.block_eight_up(self.vnet_1.block_eight(x_v1))
        x_v1 = x_v1 + x0_v1
        x_v1 = self.vnet_1.block_nine(x_v1)
        if self.has_dropout:
            x_v1 = self.vnet_1.dropout(x_v1)
        pred_VNet1 = self.vnet_1.out_conv(x_v1)

        # vnet_2 decoder
        x_v2 = self.vnet_2.block_five_up(self.vnet_2.block_five(x4_v2))
        x_v2 = x_v2 + x3_v2
        x_v2 = self.vnet_2.block_six_up(self.vnet_2.block_six(x_v2))
        x_v2 = x_v2 + x2_v2
        x_v2 = self.vnet_2.block_seven_up(self.vnet_2.block_seven(x_v2))
        x_v2 = x_v2 + x1_v2
        x_v2 = self.vnet_2.block_eight_up(self.vnet_2.block_eight(x_v2))
        x_v2 = x_v2 + x0_v2
        x_v2 = self.vnet_2.block_nine(x_v2)
        if self.has_dropout:
            x_v2 = self.vnet_2.dropout(x_v2)
        pred_VNet2 = self.vnet_2.out_conv(x_v2)

        ### Y^a
        pred_VNet1_soft = torch.softmax(pred_VNet1, dim=1)
        ### Y^b
        pred_VNet2_soft = torch.softmax(pred_VNet2, dim=1)

        entmap1 = self.get_entropy_map(pred_VNet1_soft)
        entmap2 = self.get_entropy_map(pred_VNet2_soft)

        fusion_map = self.Discriminator(pred_VNet1, pred_VNet2, pred_VNet1_soft, pred_VNet2_soft, entmap1, entmap2)

        return pred_VNet1, pred_VNet2, pred_VNet1_soft, pred_VNet2_soft, fusion_map