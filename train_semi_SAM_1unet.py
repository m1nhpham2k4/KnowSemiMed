...
from trainer_1unet import Trainer


parser = argparse.ArgumentParser()
parser.add_argument('--data_path', type=str, default='./ColonDB',
                    help='Name of Experiment')
parser.add_argument('--labeled_num', type=int, default=10,
                    help='Percentage of label quantity')

parser.add_argument('--dataset', type=str, default='/ColonDB_10',
                    help='Name of Experiment')
...
parser.add_argument('-UNet1_lr', type=float, default=0.01, help='initial learning rate')
...
parser.add_argument('--max_iterations', type=int, default=11999,
                    help='maximum epoch number to train')
...
parser.add_argument("--sam_checkpoint", type=str, default="./medsam_vit_b.pth", help="sam checkpoint")
...
def train(args, snapshot_path):
    ...
    for _ in iterator:
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            iter_num += 1
            train_metrics = trainer.train(volume_batch, label_batch, iter_num)

            if iter_num > 0 and iter_num % 200 == 0:
                if "ACDC" not in args.dataset:
                    val_metrics = trainer.val(val_loader, snapshot_path, iter_num)
                else:
                    val_metrics = trainer.val_ACDC(val_loader, snapshot_path, iter_num)

                wandb.log({
                    "train/sam_loss": train_metrics["sam_loss"],
                    "train/SGDL_loss": train_metrics["SGDL_loss"],
                    "train/UNet1_loss": train_metrics["UNet1_loss"],
                                                            "val/sam_mean_dice": val_metrics["sam_mean_dice"],
                    "val/SGDL_mean_dice": val_metrics["SGDL_mean_dice"],
                                                        }, step=iter_num)
...
if __name__ == '__main__':
    ...
    for fold in range(args.n_fold):
        ...
        snapshot_path = "./Results/ColonDB_10_MedSAM_1UNet"
        ...
        shutil.copyfile("./train_semi_SAM_1unet.py", snapshot_path + "/code/train_semi_SAM_1unet.py")
        shutil.copyfile("./trainer_1unet.py", snapshot_path + "/code/trainer_1unet.py")
        ...
        wandb.init(
            entity="minh02122004-insdustrial-university-",
            project="KnowSAM_ColonDB",
            name=f"Train_ColonDB_10_MedSAM_1UNet",
            config=vars(args)
        )
